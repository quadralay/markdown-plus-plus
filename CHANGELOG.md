...
- Reconciled § 11.3's Unset rule with production publishing behavior by replacing the unconditional "the block passes through as-is" MUST with a **processor mode** model. The pre-evaluation check is unchanged and unconditional -- when any name in an expression is Unset, a processor MUST NOT evaluate the expression -- but the fate of the unevaluated block now depends on the mode the processor declares. **Source-preserving mode** (linters, validators, converters, formatters -- tools that round-trip source) preserves the block as-is: opening tag, content, and closing tag. **Publishing mode** (processors resolving a document to a final deliverable) treats the block as Visible: content emitted, condition tags omitted, indistinguishable from an explicitly Visible condition -- correct because a published deliverable has no downstream consumer for raw condition tags, and because in production pipelines Unset is the default state of every condition a target does not explicitly configure. A processor operates in exactly one mode per run and MUST declare its mode (a processor offering both documents how the mode is selected); the mode-declaration requirement is added to § 2.2 Full Conformance and to the Processing Model's Required Features and Conformance Statement, and § 2.3's determinism guarantee now scopes sameness of output to a declared mode. Variable substitution inside the block is unchanged in both modes. A scoping note in both § 11.3 and the Processing Model's Unset Pre-Evaluation Check states that all other "passes through as-is" language in the specification and its normative references describes source-preserving mode, and that in publishing mode the block resolves as Visible during Phase 1 (includes within it expand, styles and markers within it attach) so no condition tags reach Phase 2 -- which leaves the interaction sections (§ 11.4, includes, styles, aliases, markers, tables, content islands, `cross-file-link-resolution.md`, `multiline-cell-extensions.md`, `element-interactions.md`) correct as written. § 11.3's worked example and § 11.7's Example 4 each gain a publishing-mode companion output alongside the source-preserving one. The mode names deliberately avoid "passthrough," which is already taken twice in this ecosystem (the `Passthrough` marker key and § 2.1 pass-through conformance) and a third time by ePublisher's target-level Passthrough setting. New `GLOSSARY.md` term **processor mode**; the **Unset** entry reworked to point at it ([#130](https://github.com/quadralay/markdown-plus-plus/issues/130)).

### Tooling

- Skill surfaces updated for the mode model, with the authoring takeaway stated plainly: do not rely on an Unset condition to hide or to preserve anything in published output -- if content must be dropped, define the condition and set it Hidden. `SKILL.md`'s condition state model paragraph, `references/syntax-reference.md` (pre-check statement now carries a mode table, Expression Examples rows reworded from "Pass through" to "Not evaluated," the Unset interaction table scoped to source-preserving mode with a publishing-mode note, and the variable-inside-Unset example given both outputs), `references/best-practices.md` (Unset authoring guidance rewritten around the two modes), `references/examples.md` (Example 12 walkthrough labeled source-preserving with the publishing outcome stated), `references/error-codes.md` (MDPP019's in-cell span behavior sentence), and the `tests/sample-unset-passthrough.md` fixture (header note scoping its expected outputs to source-preserving mode; the include-inside-Unset case notes the publishing divergence). `validate-mdpp.py` needed no change -- it is authoring-time and mode-agnostic, with no Unset-related behavior ([#130](https://github.com/quadralay/markdown-plus-plus/issues/130)).
