...
**Tri-state model:** Each condition name is Visible, Hidden, or Unset. Unset conditions are not evaluated — the condition block passes through as-is (opening tag, content, and closing tag preserved), allowing the implementation to resolve undefined conditions downstream.
