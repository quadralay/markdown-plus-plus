...
Complex tables with rich content in cells. Each logical row may span multiple physical lines: every pipe-bearing row continues the preceding row, and row separators (pipe-bearing rows with whitespace-only cells) delimit logical rows. A no-pipe blank line ends the table entirely.
