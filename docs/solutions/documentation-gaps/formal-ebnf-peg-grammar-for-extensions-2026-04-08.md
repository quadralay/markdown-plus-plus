...
**Three identifier productions**: `identifier ::= (letter | "_") (letter | digit | "-" | "_")*` for variables and conditions, `alias_name ::= (letter | digit | "_") (letter | digit | "-" | "_")*` for aliases (digit-first supports CMS record IDs like `#04499224`), and `style_name ::= (letter | "_") (letter | digit | "-" | "_" | " ")*` for styles and markers (embedded spaces support compound names like `Blockquote Paragraph`). These correspond to the validator's `STANDARD_NAME_RE`, `ALIAS_NAME_RE`, and `STYLE_NAME_RE`. The third production was added by #52 to correct the original two-pattern system's overreach in forbidding spaces for styles and markers. Issue [#108](https://github.com/quadralay/markdown-plus-plus/issues/108) (2026-05-22) later widened the `alias_name` production by introducing dedicated `alias_name_start_char` and `alias_name_char` productions that admit the XML 1.0 NCName letter ranges; the `identifier` and `style_name` letter classes remain ASCII pending a separate audit.
...
- `docs/solutions/documentation-gaps/spec-passthrough-marker-injection-vs-silent-discard-2026-04-11.md` -- Follow-up: corrected the "unrecognized segments silently discarded" prose description (referenced in Symptoms above as the pre-grammar state) to implementation-defined disposition across `specification.md`, `formal-grammar.md`, and `syntax-reference.md`
- `docs/solutions/tooling-decisions/unicode-alias-letter-class-via-ncname.md` -- Follow-up: introduced `alias_name_start_char` and `alias_name_char` productions citing XML 1.0 NCName ranges, replacing the original ASCII-only `alias_name` body shown above (2026-05-22, #108)
