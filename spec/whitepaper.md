...
The degree of invisibility varies by extension type. Most comment-based directives (styles, aliases, markers, combined commands, inline styles, content islands) are fully hidden -- standard renderers treat HTML comments as invisible per the CommonMark specification. Conditions and multiline tables are also comment-based, but their content renders differently: all conditional branches appear simultaneously, and continuation rows display as separate table rows. Inline token extensions (variables) pass through as literal text, so a reader sees `$product_name;` rather than the resolved value. And content that depends on processing -- included files and cross-file link resolution -- is absent in standard views. For the full per-extension breakdown, see the [Graceful Degradation appendix](graceful-degradation.md).
...
- **Conditional content** -- `<!-- condition:online -->...<!-- /condition -->` shows or hides content based on output format, audience, or any defined condition. The directive wraps content inline, contrasting with attribute-based filter systems used by XML formats.
...
Conditional content illustrates a deeper structural difference between the two formats. Markdown++ wraps content with inline directives that sit next to the content they gate. DITA tags content with attributes (`@audience`, `@platform`, `@product`) and resolves them at build time against separate DITAVAL filter files. The practical consequences fall along three lines. Locality: a Markdown++ condition lives inside the source file with the content it controls, while a DITA condition splits across content attributes and the DITAVAL rules that interpret them. Diff visibility: adding or removing a Markdown++ condition is a single-location change with a clear diff, while the DITA equivalent typically pairs attribute changes in content with rule changes in DITAVAL across separate files. Authoring ergonomics: a writer editing a Markdown++ file sees what is conditional at a glance, while a writer editing a DITA topic sees attribute markup and reasons about how each filter file resolves it. Both approaches solve the same problem; the design choice is whether condition logic should live with the content or in a separate rule file.
