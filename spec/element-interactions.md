...
Document order is determined by the assembled document after Phase 1 processing (include expansion, defined conditions evaluated (Unset condition blocks pass through as-is), and variable substitution). For multi-file documents, this follows the depth-first recursive include expansion order defined in the [Processing Model](processing-model.md).
