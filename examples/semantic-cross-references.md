...
When the alias is hand-authored from a publishing tool's numeric registry, the slug supplies the human-readable handle the alias lacks -- and the two values look different on purpose. When the alias is itself a unique-by-construction semantic identifier (generated by an authoring agent, a pipeline minting contextual IDs, or a hash-derived slug scheme), the link reference definition can reuse the alias value as its slug. Fewer identifiers per heading, no parallel naming vocabulary to maintain, and the alias becomes the single source of truth for the heading.
