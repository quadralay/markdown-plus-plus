...
A condition state for a name that is not defined in the condition set. Each condition name has one of three states: **Visible** and **Hidden** are assigned states (set in the condition set provided at build time); **Unset** means the condition name is not defined in the condition set. Before evaluating a condition expression, a conformant processor MUST check whether all condition names are defined; if any operand is Unset, the processor MUST NOT evaluate the expression. What happens to the unevaluated block depends on the processor's declared [processor mode](#processor-mode): a source-preserving processor passes the block through as-is (opening tag, content, and closing tag preserved), while a publishing processor treats the block as Visible (content emitted, condition tags omitted). In both modes variable substitution still applies to the block's content because the content survives Phase 1, Step 1 into Phase 1, Step 2.
...
Which of the two specified behaviors a conformant processor implements for an [Unset](#unset) condition block. **Source-preserving mode** -- for linters, validators, converters, formatters, and other tools that round-trip Markdown++ source -- passes the block through as-is, preserving the opening tag, content, and closing tag. **Publishing mode** -- for processors that resolve a document to a final published deliverable -- treats the block as Visible, emitting the content and omitting the condition tags, because a published deliverable has no downstream consumer for raw condition tags and Unset is commonly the default state of any condition a target does not explicitly configure. The Unset pre-evaluation check itself is identical in both modes (the expression is never evaluated), as is variable substitution inside the block; the modes differ only in whether the condition tags survive. A processor operates in exactly one mode for a given processing run; a conformant processor MUST declare its mode in its conformance documentation, and a processor that offers both modes documents how the mode is selected. Not to be confused with the `Passthrough` marker key or with pass-through conformance (a renderer that preserves all Markdown++ extensions unevaluated).

Full treatment: [Section 11.3 Unset Pre-Evaluation Check](spec/specification.md#unset-pre-evaluation-check).
